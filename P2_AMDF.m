...
[ pointsX_3level , pointsY_3level ] = calcute_fc(candidate_3lvl_cp,num_of_window);
[ pointsX_cp , pointsY_cp ] = calcute_fc(candidate_cp,num_of_window);
...
xlim([0 num_of_window])
ylim([50 200])
title("AMDF Function , Frame Length="+string(N)+"ms , Number Of Windows="+string(num_of_window));
...
function [ pointsX , pointsY ] = calcute_fc(final_candidate,num_of_window)
    ...
    final_candidate = smooth2a(final_candidate,20,10);
...
function locs = find_locs(y)
    p = smooth(smooth(smooth(y,10),10),50) ;
    p = -p;
...
function amdf_f = amdf(y)
    amdf_f = zeros(1,length(y));
    for k=0:length(y)-1
        sum=0;
        for j=k:length(y)-1
            sum =  sum + abs((y(j+1)-y(j+1-k)));
        end
        amdf_f(k+1) = sum;
    end
end
